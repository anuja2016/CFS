<?php
include_once 'header.php';
?>
<section class="banner-block-lft mt-116 margin-btm">
    <div class="block-img">
        <img src="../assets/images/banners/feature_banner_scheduling.jpg" class="img-fluid desk-homebanner" alt="features-banner">
        <img src="../assets/images/banners/feature_banner_scheduling_mob.jpg" class="img-fluid mob-homebanner" alt="features-banner" />
        <!-- <div class="overlay"></div> -->
    </div>
    <div class="main-Wrapper d-flex justify-content-end">
        <div class="content">
            <h1>Schedule customer complaints with scheduling management software</h1>
            <p>FieldWeb accelerates job scheduling, minimizes booking errors, and optimizes route planning, ensuring seamless coordination between your field team and clients. With FieldWeb, efficiently manage field service tasks, enhance customer satisfaction, and boost productivity.</p>
            <div class="sect-getservice">
                <div class="input-group">
                    <span class="input-group-text" id="basic-addon1">
                        <select class="form-control">
                            <option>India</option>
                            <option>Albania</option>
                            <option>Algeria</option>
                        </select>
                    </span>
                    <input type="text" class="form-control" placeholder="Enter Your mobile number " aria-label="MobileNumber">
                </div>
                <button type="submit" class="btn-getstarted">Get Started</button>
            </div>
        </div>
    </div>
</section>
<section class="main-Wrapper margin-btm" data-aos="fade-up">
    <div class="mb-3 text-center">
        <h3 class="title-md-montbold"> FieldWeb ensures seamless job scheduling in real-time.</h3>
    </div>
    <div class="job-list mt-4">
        <ul>
                 <li><a href="quotation.php">Quote</a></li>
            <li><a href="Scheduling.php">Schedule</a></li>
            <li><a href="complete-job.php">Complete Job</a></li>
            <li><a href="invoice.php">Invoice</a></li>
            <li><a href="lead-management.php">Lead Management</a></li>
        </ul>
        </ul>
    </div>
</section>
<section class="block-grey-md margin-btm" data-aos="fade-up">
    <div class="text-center main-Wrapper">
        <h3 class="title-md-montbold mb-3">How do quotes work in FieldWeb?</h3>
        <p>With these quoting tools, FieldWeb makes it simpler for you to manage your time effectively, secure more projects, and provide the polished and hassle-free service that your clients are looking for.</p>
    </div>
</section>
<section class="custom-tabs sect-fd-tabs margin-btm" data-aos="fade-up">
    <div class="main-Wrapper">
        <div class="margin-btm">
            <ul class="d-flex gap-4 justify-content-center">
                <li>
                    <a id="btn-BookJob" class="ref-link">Book Job</a>
                </li>
                <li>
                    <a id="btn-JobAssignment" class="ref-link">Job Assignment</a>
                </li>
                <li>
                    <a id="btn-JobStatuses" class="ref-link">Job Statuses</a>
                </li>
                <li>
                    <a id="btn-JobScheduling" class="ref-link">Job Scheduling </a>
                </li>
                <li>
                    <a id="btn-JobReminders" class="ref-link">Job Reminders</a>
                </li>
                <li>
                    <a id="btn-Map" class="ref-link">Map</a>
                </li>
            </ul>
        </div>
        <div class="margin-btm" id="tb-BookJob">
            <div class="row align-items-center flex-row-reverse">
                <div class="col-md-6">
                    <img src="../assets/images/features/book_job_online.jpg" class="img-fluid">
                </div>
                <div class="col-md-6">
                    <p class="title-sm-red mb-2">Book Job Online</p>
                    <h2 class="title-bold">Streamline Scheduling and Dispatching with FieldWeb's Online Booking Feature</h2>
                    <div class="text-justify">
                        <p class="mb-4">Book jobs seamlessly through FieldWeb's customer platform, enhancing customer convenience and reducing response time. Simplify scheduling and dispatching by enabling customers to request services, set preferred timings, and receive real-time updates.</p>
                        <p class="mb-4"> This feature improves efficiency, minimizes errors, and strengthens customer engagement, ensuring a smooth and hassle-free experience for both businesses and clients</p>
                    </div>
                    <div class="mb-3 mt-4">
                        <a data-bs-toggle="modal" data-bs-target="#getdemoModal" class="btn-red arrowBtn u-button has-hover">
                            <span class="u-button-arrow"></span>
                            <span>Get Demo</span>
                        </a>
                    </div>
                </div>
            </div>
        </div>
        <div class="margin-btm" id="tb-JobAssignment">
            <div class="row align-items-center">
                <div class="col-md-6">
                    <img src="../assets/images/features/bulk_job_assignment.jpg" class="img-fluid">
                </div>
                <div class="col-md-6">
                    <p class="title-sm-red mb-2">Bulk Job Assignment</p>
                    <h2 class="title-bold">Enhance Workload Management with FieldWeb's Bulk Job Assignment Feature</h2>
                    <div class="text-justify">
                        <p class="mb-4">Maximize Efficiency: Simplify Workload Distribution with FieldWeb's Bulk Job Assignment Feature. Seamlessly assign multiple jobs in one go, ensuring optimal resource allocation and organized team management.</p>
                        <p class="mb-4"> Keep your operations responsive and your workforce coordinated for streamlined success. </p>
                    </div>
                    <div class="mb-3 mt-4">
                        <a data-bs-toggle="modal" data-bs-target="#getdemoModal" class="btn-red arrowBtn u-button has-hover">
                            <span class="u-button-arrow"></span>
                            <span>Get Demo</span>
                        </a>
                    </div>
                </div>
            </div>
        </div>
        <div class="margin-btm" id="tb-JobStatuses">
            <div class="row align-items-center flex-row-reverse">
                <div class="col-md-6">
                    <img src="../assets/images/features/job_status_at_a_glance.jpg" class="img-fluid">
                </div>
                <div class="col-md-6">
                    <p class="title-sm-red mb-2">Job Statuses at a Glance</p>
                    <h2 class="title-bold">Effortless Job Progress Monitoring with FieldWeb: Track Job Statuses Seamlessly</h2>
                    <div class="text-justify">
                        <p class="mb-4">FieldWeb empowers you to effortlessly monitor and manage job progress. Our intuitive platform enables you to track various job statuses including ongoing, inactive, on hold, rejected, and completed. </p>
                        <p class="mb-4">This comprehensive overview ensures streamlined operations and effective decision-making for your business.</p>
                    </div>
                    <div class="mb-3 mt-4">
                        <a data-bs-toggle="modal" data-bs-target="#getdemoModal" class="btn-red arrowBtn u-button has-hover">
                            <span class="u-button-arrow"></span>
                            <span>Get Demo</span>
                        </a>
                    </div>
                </div>
            </div>
        </div>
        <div class="margin-btm" id="tb-JobScheduling">
            <div class="row align-items-center">
                <div class="col-md-6">
                    <img src="../assets/images/features/smart_job_scheduling.jpg" class="img-fluid">
                </div>
                <div class="col-md-6">
                    <p class="title-sm-red mb-2">Smart Job Scheduling</p>
                    <h2 class="title-bold">Plan Ahead with FieldWeb: Empower Your Business with Advanced Scheduling</h2>
                    <div class="text-justify">
                        <p class="mb-4">FieldWeb's cutting-edge scheduling features put you in control of your operations. Seamlessly schedule jobs in advance, enabling your field workers to optimize their time and resources for peak efficiency.  </p>
                        <p class="mb-4">By providing a proactive approach to planning, FieldWeb ensures smoother workflows and enhanced customer satisfaction.</p>
                    </div>
                    <div class="mb-3 mt-4">
                        <a data-bs-toggle="modal" data-bs-target="#getdemoModal" class="btn-red arrowBtn u-button has-hover">
                            <span class="u-button-arrow"></span>
                            <span>Get Demo</span>
                        </a>
                    </div>
                </div>
            </div>
        </div>
        <div class="margin-btm" id="tb-JobReminders">
            <div class="row align-items-center flex-row-reverse">
                <div class="col-md-6">
                    <img src="../assets/images/features/job_reminders.jpg" class="img-fluid">
                </div>
                <div class="col-md-6">
                    <p class="title-sm-red mb-2">Job Reminders</p>
                    <h2 class="title-bold">Stay on Top of Tasks with FieldWeb: Reliable Job Reminders for Seamless Operations</h2>
                    <div class="text-justify">
                        <p class="mb-4">FieldWeb's job reminder functionality is your safety net for flawless task management. Benefit from timely notifications and updates, ensuring your team meets deadlines and upholds top-notch service quality. These reminders streamline your operations, preventing oversights and enhancing overall efficiency. With FieldWeb, you can count on a reliable system that keeps your team aligned and your customers satisfied.</p>
                     
                    </div>
                    <div class="mb-3 mt-4">
                        <a data-bs-toggle="modal" data-bs-target="#getdemoModal" class="btn-red arrowBtn u-button has-hover">
                            <span class="u-button-arrow"></span>
                            <span>Get Demo</span>
                        </a>
                    </div>
                </div>
            </div>
        </div>
        <div class="margin-btm" id="tb-Map">
            <div class="row align-items-center">
                <div class="col-md-6">
                    <img src="../assets/images/features/map_and_route_optimization.jpg" class="img-fluid">
                </div>
                <div class="col-md-6">
                    <p class="title-sm-red mb-2">Map and Routing Optimization</p>
                    <h2 class="title-bold">Optimize Travel Routes with FieldWeb: Enhanced Efficiency Through Mapping and Routing</h2>
                    <div class="text-justify">
                        <p class="mb-4">FieldWeb's map and routing tools are your navigational allies for efficient travel. Monitor your workforce's daily routes and distances traveled, allowing for smart route optimization and minimized travel durations. This feature not only saves time but also cuts down on operational costs. With FieldWeb, your team can achieve more tasks in less time, enhancing overall productivity and elevating customer satisfaction.</p>
                        
                    </div>
                    <div class="mb-3 mt-4">
                        <a data-bs-toggle="modal" data-bs-target="#getdemoModal" class="btn-red arrowBtn u-button has-hover">
                            <span class="u-button-arrow"></span>
                            <span>Get Demo</span>
                        </a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<section class="main-Wrapper margin-btm" data-aos="fade-up">
    <div class="sect-jobquote text-justify">
        <h2 class="title text-center">What is Job Scheduling Software?</h2>
        <p class="mt-3 mb-4">FieldWeb's scheduling software revolutionizes how you book and manage field service tasks. Craft your personalized calendar, effortlessly allocate jobs to the right experts, and seamlessly communicate appointment particulars with both your team and clients.</p>
        <ul>
            <li>Discover the FieldWeb Way of Scheduling:</li>

            <li><img src="../assets/images/arrowrightblack.svg" class="img-fluid">Stay informed about online bookings or directly create appointments within your calendar</li>
            <li><img src="../assets/images/arrowrightblack.svg" class="img-fluid">Effortlessly assess team availability and allocate tasks to the suitable members</li>
            <li><img src="../assets/images/arrowrightblack.svg" class="img-fluid">Simply click, drag, and realign appointments that require rescheduling</li>
            <li><img src="../assets/images/arrowrightblack.svg" class="img-fluid">Prioritize efficiency by automatically generating optimal driving routes before deploying your team</li>
            <li><img src="../assets/images/arrowrightblack.svg" class="img-fluid">Enhance customer experience by setting up automated reminders that prep clients ahead of their appointments</li>
            
        </ul>
    </div>
</section>
<section class="main-Wrapper margin-btm" data-aos="fade-up">
    <div class="row">
        <div class="col-md-6">
            <div class="quote-grey-block p-4 m-mb-5">
                <h5 class="block-red text-center">For your Business:</h5>
                <ul class="text-justify title-sm-para">
                    <li class="pt-3">Job scheduling software streamlines business operations by providing a centralized platform to manage appointments</li>
                    <li class="pt-3">Efficiently assign tasks to the appropriate team members based on their expertise and availability</li>
                    <li class="pt-3">Gain valuable insights into your business's performance by tracking scheduled jobs, completed tasks, and pending appointments. </li>
                </ul>
            </div>
        </div>
        <div class="col-md-6">
            <div class="quote-grey-block p-4">
                <h5 class="block-red text-center">For your Customer:</h5>
                <ul class="text-justify title-sm-para">
                    <li class="pt-3">Your customers can easily request appointments online through the software, eliminating the need for lengthy phone calls.</li>
                    <li class="pt-3">Keep your customers informed with real-time updates about their scheduled appointments, including confirmation, reminders, and any changes.</li>
                    <li class="pt-3">ob scheduling software enables customers to choose appointment slots that best suit their schedules.</li>
                </ul>
            </div>
        </div>
    </div>
</section>
<section class="margin-btm learnmore-block-outer" data-aos="fade-up">
    <div class="main-Wrapper">
        <div class="learnmore-block">
            <p>BOOK YOUR FREE DEMO</p>
            <h3>Learn more about <span>FieldWeb</span></h3>
            <button type="submit" class="btn-getdemo-wh" data-bs-toggle="modal" data-bs-target="#getdemoModal">Get Demo <img src="../assets/images/arrowrightblack.svg" /></button>
        </div>
    </div>
</section>
<section class="text-center margin-btm" data-aos="fade-up">
    <div class="wrapper-head text-center margin-btm">
        <p class="title-red">OUR POWERFUL FEATURES</p>
        <h2 class="title">I Switched to FieldWeb<p></p>
        </h2>
    </div>
    <div class="main-Wrapper">
        <div class="client-review mt-5">
            <div class="review-card">
                <div class="review-card-content">
                    <p>FieldWeb is such a great app for track the technicians and their work.</p>
                    <p>Wonderful Support from Team.</p>
                    <p>Thanks</p>
                    <h4 class="title-poppins-sbold">Nextgen Technologies</h4>
                    <h5>Arendhu</h5>
                    <div class="sect-rating">
                        <ul>
                            <li class="full-star"><img src="../assets/images/full-star-yellow.svg"></li>
                            <li class="full-star"><img src="../assets/images/full-star-yellow.svg"></li>
                            <li class="full-star"><img src="../assets/images/full-star-yellow.svg"></li>
                            <li class="full-star"><img src="../assets/images/full-star-yellow.svg"></li>
                            <li class="half-star"><img src="../assets/images/half-star-yellow.svg"></li>
                        </ul>
                    </div>
                </div>
            </div>
            <div class="review-card">
                <div class="review-card-content">
                    <p>FieldWeb is such a great app for track the technicians and their work.</p>
                    <p>Wonderful Support from Team.</p>
                    <p>Thanks</p>
                    <h4 class="title-poppins-sbold">Splash RO System</h4>
                    <h5>Mohit Sapra</h5>
                    <div class="sect-rating">
                        <ul>
                            <li class="full-star"><img src="../assets/images/full-star-yellow.svg"></li>
                            <li class="full-star"><img src="../assets/images/full-star-yellow.svg"></li>
                            <li class="full-star"><img src="../assets/images/full-star-yellow.svg"></li>
                            <li class="full-star"><img src="../assets/images/full-star-yellow.svg"></li>
                            <li class="half-star"><img src="../assets/images/half-star-yellow.svg"></li>
                        </ul>
                    </div>
                </div>
            </div>
            <div class="review-card">
                <div class="review-card-content">
                    <p>FieldWeb is such a great app for track the technicians and their work.</p>
                    <p>Wonderful Support from Team.</p>
                    <p>Thanks</p>
                    <h4 class="title-poppins-sbold">Godavari Mineral Water Technologies</h4>
                    <h5>Chiranjeevi Rao</h5>
                    <div class="sect-rating">
                        <ul>
                            <li class="full-star"><img src="../assets/images/full-star-yellow.svg"></li>
                            <li class="full-star"><img src="../assets/images/full-star-yellow.svg"></li>
                            <li class="full-star"><img src="../assets/images/full-star-yellow.svg"></li>
                            <li class="full-star"><img src="../assets/images/full-star-yellow.svg"></li>
                            <li class="half-star"><img src="../assets/images/half-star-yellow.svg"></li>
                        </ul>
                    </div>
                </div>
            </div>
            <div class="review-card">
                <div class="review-card-content">
                    <p>FieldWeb is such a great app for track the technicians and their work.</p>
                    <p>Wonderful Support from Team.</p>
                    <p>Thanks</p>
                    <h4 class="title-poppins-sbold">Image India</h4>
                    <h5>Prasad Raju Kalindi</h5>
                    <div class="sect-rating">
                        <ul>
                            <li class="full-star"><img src="../assets/images/full-star-yellow.svg"></li>
                            <li class="full-star"><img src="../assets/images/full-star-yellow.svg"></li>
                            <li class="full-star"><img src="../assets/images/full-star-yellow.svg"></li>
                            <li class="full-star"><img src="../assets/images/full-star-yellow.svg"></li>
                            <li class="half-star"><img src="../assets/images/half-star-yellow.svg"></li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<section class="mt-10 margin-btm" data-aos="fade-up">
    <div class="main-Wrapper">
        <div class="text-center margin-btm">
            <h2 class="title-semibold">Big & Small Business Trusted Us<h2>
        </div>
        <div class="owl-carousel owl-theme" id="TrustedCompaniesCarousel">
            <div class="item">
                <img src="../assets/images/gopienterprises.svg" class="partner-img">
            </div>
            <div class="item">
                <img src="../assets/images/nextgenlogo.svg" class="partner-img">
            </div>
            <div class="item">
                <img src="../assets/images/njclogo.svg" class="partner-img">
            </div>
            <div class="item">
                <img src="../assets/images/gopienterprises.svg" class="partner-img">
            </div>
            <div class="item">
                <img src="../assets/images/nextgenlogo.svg" class="partner-img">
            </div>
            <div class="item">
                <img src="../assets/images/njclogo.svg" class="partner-img">
            </div>
        </div>
        <div class="owl-carousel owl-theme mt-5" id="TrustedCompaniesCarousel2">
            <div class="item">
                <img src="../assets/images/gopienterprises.svg" class="partner-img">
            </div>
            <div class="item">
                <img src="../assets/images/nextgenlogo.svg" class="partner-img">
            </div>
            <div class="item">
                <img src="../assets/images/njclogo.svg" class="partner-img">
            </div>
            <div class="item">
                <img src="../assets/images/gopienterprises.svg" class="partner-img">
            </div>
            <div class="item">
                <img src="../assets/images/nextgenlogo.svg" class="partner-img">
            </div>
            <div class="item">
                <img src="../assets/images/njclogo.svg" class="partner-img">
            </div>
        </div>
</section>
<!----------get demo----------->
<section class="sect-demo mt-20 pb-5" data-aos="fade-up">
    <div class="block-lightorange">
        <div class="row align-items-center">
            <div class="col-md-6">
                <p class="subtitle">LEADING All -IN ONE SOFTWARE</p>
                <h2 class="black-xl-title mt-3 mb-4">Book your <span class="drawn-underline">Personalized</span> Demo to learn more</h2>
                <p class="subtext">Explore why 1000 + service professional trust FieldWeb to run their business.</p>
            </div>
            <div class="col-md-5 ms-auto">
                <div class="card">
                    <div class="card-body">
                        <div class="modal-header">
                            <h6 class="title-poppins-sbold">Request a personalized demo</h6>
                        </div>
                        <div class="row mb-3 mt-3">
                            <div class="col-md-12">
                                <input type="email" class="form-control" placeholder="Email">
                            </div>
                            <div class="col-md-12">
                                <input type="text" class="form-control mt-2" placeholder="First Name">
                            </div>
                            <div class="col-md-12">
                                <input type="number" class="form-control mt-2" placeholder="Mobile Number*">
                            </div>
                            <div class="col-md-12">
                                <input type="text" class="form-control mt-2" placeholder="Company Name*">
                            </div>
                            <div class="col-md-12 mt-2">
                                <div class="grey-ouline-block">
                                    <div class="d-flex-common">
                                        <p class="black-sm-title">Select Industry</p>
                                        <img src="../assets/images/rightarrow.svg" />
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-12 mt-2">
                                <div class="grey-ouline-block">
                                    <div class="d-flex-common">
                                        <p class="black-sm-title">Select Number of Techs</p>
                                        <img src="../assets/images/rightarrow.svg" />
                                    </div>
                                </div>
                            </div>

                        </div>
                        <div class="text-center mb-3 mt-4">
                            <a class="btn-red arrowBtn u-button has-hover m-auto">
                                <span class="u-button-arrow"></span>
                                <span>SUBMIT</span>
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>


<?php
include_once 'footer.php';
?>
